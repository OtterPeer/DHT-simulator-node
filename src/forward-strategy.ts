...
export class DefaultForwardStrategy implements ForwardStrategy {
  async forward(
    sender: string,
    recipient: string,
    message: MessageDTO,
    ...
    try {
      for (const node of closest) {
        if (node.id !== sender && node.id !== nodeId) {
...
export class DistanceBasedForwardStrategy implements ForwardStrategy {
  async forward(
    sender: string,
    recipient: string,
    message: MessageDTO,
